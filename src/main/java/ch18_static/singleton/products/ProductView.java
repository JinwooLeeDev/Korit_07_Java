...
public class ProductView {
    // 정적 변수 선언
    // 보통 실글톤 패턴 만들 때 정적 변수 이름이 instance입니다. -> 이게 시험 시 힌트 중 하나였습니다.
    ...
    //static 메서드의 정의 -> 대부분의 경우 getInstance() -> 시험 시 힌트였습니다. 클래스명.메서드명() 호출
    public static ProductView getInstance() {
        if(instance==null) {                // 현재 인스턴스가 없는 상황이라면 20번 코드라인이 true이므로
            instance = new ProductView();   // ProductView() 생성자를 호출하여 instace에 대입함
        }
        return instance;                    // 이미 생성되었으면 바로 23번 라인이 실행됨
    }
}
